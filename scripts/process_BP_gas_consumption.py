...
if __name__ == '__main__':
    ...
    # reshape dataframe
    df = df_wide_to_long(df, value_name='gas_consumption_EJ', var_name='year')
    ...
    df_out['id'] = df_out.apply(
        lambda row: f"BP_review2022:gas-consumption:{row['actor_id']}:{row['year']}", axis=1)
    ...
    # convert to csv
    df_out.to_csv(f'{outputDir}/BP_review_2022_gas_consumption.csv', index=False)